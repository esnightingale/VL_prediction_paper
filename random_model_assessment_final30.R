setwd("H:/VL/Monthly Prediction/surveillance forecasting/model selection 0119/Results")
library(hhh4addon)
library(rlist)
library(MASS)
library(ggplot2)

source("H:/VL/Monthly Prediction/analysis data/data_run.R")
...
randmod1<-random.model(stsobj, 1)
summary(randmod1)
randmod2<-random.model(stsobj, 1)
summary(randmod2)
...
# Load 30 random mods with covariate options
...
result_table<-cbind(result_table,scores_first_avg,scores_rolling_avg,calib_all) #,ncms_all)
...
save(result_table,file="result_table_randmods30.Rdata")

summary(randmods[[1]][[20]])
for (i in 1:30){print(head(randmods[[1]][[i]]$control$ne$lag))}
