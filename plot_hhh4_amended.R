...
plot_hhh42<-function (x, units = 1, names = NULL, col = c("grey85", "blue", 
                                              "orange"), pch = 19, pt.cex = 0.6, pt.col = 1, par.settings = list(), 
          legend = TRUE, legend.args = list(), legend.observed = FALSE, 
          decompose = NULL, meanHHH = NULL, ...) 
{
  if (is.null(units)) 
    units <- seq_len(x$nUnit)
  if (!is.null(names)) 
    stopifnot(length(units) == length(names))
  if (isTRUE(decompose)) 
    decompose <- colnames(x$stsObj)
  if (is.null(meanHHH)) {
    meanHHH <- if (is.null(decompose)) {
      surveillance:::meanHHH(x$coefficients, terms(x))
    }
    else {
  ...
  col <- if (is.null(decompose) && length(col) == 4) {
    ...
    surveillance:::plotHHH4_fitted_check_col_decompose(col, 
                                                       decompose)
  }
  if (is.list(par.settings)) {
    par.defaults <- list(mfrow = sort(n2mfrow(length(units))), 
                         mar = c(4, 4, 2, 0.5) + 0.1, las = 1)
    par.settings <- modifyList(par.defaults, par.settings)
    opar <- do.call("par", par.settings)
    on.exit(par(opar))
  }
  ...
  if (length(legend) > 0) {
    ...
    default.args <- list(x = "topright", col = c(pt.col, 
                                                 rev(col))[legendidx], lwd = 6, lty = c(NA, rep.int(1, 
                                                                                                    length(col)))[legendidx], pch = c(pch, rep.int(NA, 
                                                                                                                                                   length(col)))[legendidx], pt.cex = pt.cex, pt.lwd = 1, 
                         bty = "n", inset = 0.02, legend = if (is.null(decompose)) {
                           c("observed", "spatiotemporal", "autoregressive", "endemic")[legendidx]
                         } else c("observed", rev(decompose), "endemic")[legendidx])
    legend.args <- modifyList(default.args, legend.args)
  }
  meanHHHunits <- vector(mode = "list", length = length(units))
  names(meanHHHunits) <- if (is.character(units)) 
    units
  else colnames(x$stsObj)[units]
  for (i in seq_along(units)) {
    meanHHHunits[[i]] <- plotHHH4_fitted2(x, 
                                                         units[i], main = names[i], col = col, pch = pch, 
                                                         pt.cex = pt.cex, pt.col = pt.col, decompose = decompose, 
                                                         meanHHH = meanHHH, ...)
    if (i %in% legend) 
      do.call("legend", args = legend.args)
  }
  invisible(meanHHHunits)
}
...
plotHHH4_fitted2<-function (x, unit = 1, main = NULL, col = c("grey85", "blue", 
                                            "orange"), pch = 19, pt.cex = 0.6, pt.col = 1, border = col, 
          start = x$stsObj@start, end = NULL, xaxis = NULL, xlim = NULL, 
          ylim = NULL, xlab = "", ylab = "No. infected", hide0s = FALSE, 
          decompose = NULL, total = FALSE, meanHHH = NULL) 
{
  stsObj <- x$stsObj
  if (!total && is.character(unit) && is.na(unit <- match(.unit <- unit, 
                                                          colnames(stsObj)))) 
    stop("region '", .unit, "' does not exist")
  if (is.null(main)) 
    main <- if (total) 
      "Overall"
  else colnames(stsObj)[unit]
  if (isTRUE(decompose)) 
    decompose <- colnames(stsObj)
  obs <- if (total) 
    rowSums(observed(stsObj))
  else observed(stsObj)[, unit]
  start0 <- yearepoch2point(stsObj@start, stsObj@freq, toleft = TRUE)
  start <- yearepoch2point(start, stsObj@freq)
  ...
  if (start < start0 || start > tp[length(tp)]) 
    ...
    tp[length(tp)]
  else yearepoch2point(end, stsObj@freq)
  stopifnot(start < end)
  tpInRange <- which(tp >= start & tp <= end)
  tpInSubset <- intersect(x$control$subset, tpInRange)
  if (is.list(xaxis)) {
    tp <- seq_along(obs)
    start <- tpInRange[1L]
    end <- tpInRange[length(tpInRange)]
  ...
  meanHHHunit <- if (is.null(decompose)) {
    ...
    if (total) {
      apply(meanHHH[, , c("endemic", decompose)], c(1L, 
                                                    3L), sum)
    }
    else {
      meanHHH[, unit, c("endemic", decompose)]
  ...
  stopifnot(is.matrix(meanHHHunit), !is.null(colnames(meanHHHunit)), 
            nrow(meanHHHunit) == length(x$control$subset))
  meanHHHunit <- meanHHHunit[x$control$subset %in% tpInRange, 
                             , drop = FALSE]
  if (any(is.na(meanHHHunit))) {
    stop("predicted mean contains missing values")
  }
  ...
  if (is.null(ylim)) 
    ylim <- c(0, max(obs[tpInRange], na.rm = TRUE))
  plot(c(start, end), ylim, xlim = xlim, xlab = xlab, ylab = ylab, 
       type = "n", xaxt = if (is.list(xaxis)) 
         "n"
       else "s")
  if (is.list(xaxis)) 
 ...
 # return(meanHHHunit)
  ...
  if (is.null(decompose)) {
    non0 <- which(c("end", "ar", "ne") %in% componentsHHH4(x))
   # print(non0)
    plotComponentPolygons(x = tp[tpInSubset], y = meanHHHunit[, 
                                                              c("endemic", "epi.own", "epi.neighbours")[non0], 
                                                              drop = FALSE], col = col[non0], border = border[non0], 
                          add = TRUE)
    lines(x = tp[tpInSubset], y = meanHHHunit[,"mean"],lwd=2,col="black")
  }
  else {
    non0 <- apply(X = meanHHHunit > 0, MARGIN = 2L, FUN = any)
    plotComponentPolygons(x = tp[tpInSubset], y = meanHHHunit[, 
                                                              non0, drop = FALSE], col = col[non0], border = border[non0], 
                          add = TRUE)
  }
  ptidx <- if (hide0s) 
    intersect(tpInRange, which(obs > 0))
  else tpInRange
  points(tp[ptidx], obs[ptidx], col = pt.col, pch = pch, cex = pt.cex)
  invisible(meanHHHunit)
